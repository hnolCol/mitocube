...
pingouinColumn = {"SS": "Sum of squares",
                  "F": "F-values",
                  "MS": "Mean squares",
                  "DF": "Degree of freedom",
                  "p-unc": "p-value (uncorrected)"}
...
class OldDBFeatures:
    """
    Loads and assigns annotations to features.
    """
...
class OldDataset:
    """
    Dataclass that holds the actually data (features x samples), the dataID,
    ...
    def annotateData(self) -> pd.DataFrame:
        ...
        return pd.concat([self.__getDataObj(), DB], join="inner", axis=1).dropna(how="all", axis=1)

    def getIsInMask(self, featureIDs) -> Tuple[np.ndarray, bool]:
        """Returns mask and true if featureIDs were found in data.index (bool)"""

        boolIdx = self.__getDataObj().index.isin(featureIDs)
    ...
    def getCorrelatedFeatures(self,
                              ...
                              scale: bool = True,
                              N: int = 20,
                              minValidValues: int = 4) -> Dict[str, Any]:
        ...
        if featureDetected:
            ...
            expColumnsWithNonNan = self.__getDataObj().loc[boolIdx, expColumns].dropna(axis=1).columns.values
            if expColumnsWithNonNan.size > 4:
                featureDataTransposed = pd.Series(self.__getDataObj().loc[boolIdx, expColumnsWithNonNan].values.T.flatten(), index=expColumnsWithNonNan)
                dataWhereFeatureIsNotNaN = self.__getDataObj().dropna(subset=expColumnsWithNonNan, thresh=minValidValues)
                ...
                correlatedFeaturesData = self.__getDataObj().loc[corrFeaturesIndex, ]
    ...
    def getExperimentalInformation(self, paramNames: list, joinListItems: bool = False) -> Tuple[bool, List]:
        ...
        if paramNames is not None:
            ...
            if not any(paramName in self.__getParamsObj() for paramName in paramNames):
            ...
            return False, ["Api-config might misses param 'experiment-procedure-params' (default)"]
    ...
    def getExpressionColumns(self) -> List[str]:
        ...
        if len(groupingNames) == 1:
            ...
            for n, groupingName in enumerate(groupingNames):
                if n == 2:
                    break  # limited to groupings == 2
                sampleGrouping.loc[:, f"Grouping{n}"] = self.mapGroupingsToSampleNames(groupMapper,
                                                                                       groupingName,
                                                                                       sampleNames)
                # factorize groupings to position in grouping  to allow sorting after occurrence
    ...
    def getFeatures(self) -> np.ndarray: 
        """"""
        return self.__getDataObj().index.unique().values
    ...
    def getGroupingMapper(self) -> Dict[str, dict]:
        ...
        self.groupingMapper = OrderedDict()  # ToDo: !!! move/copy to __init__
    ...
    def getGroupsByGroupingName(self, groupingName: str) -> List[str]:
        """Returns the groupNames for a specific grouping"""
        groupings = self.getGroupings()
    ...
    def getMeltedData(self,
        ...
        featureData = self.__getDataObj().loc[boolIdx]
        # save index as idx
        ...
        if addGroupings:
            ...
            for groupingName, mapper in groupingMapper.items():
                # TODO: COUNTERCHECK WHERE I have to change variable to label
                if "label" in meltedData:
    ...
    def getParams(self) -> dict:
        ...
        return self.__getParamsObj()

    def getParam(self, paramName) -> Any:
        """"""
        return self.__getParamsObj().get(paramName)
    ...
    def hasParam(self, paramName) -> bool:
        """"""
        return paramName in self.__getParamsObj()

    def mapGroupingsToSampleNames(self,
                                  groupMapper: dict, groupingName: str,
                                  expressionColumnNames: List[str]) -> List[str]:
        """"""
    ...
    def isFeatureInDataset(self, featureID: str) -> bool:
        ...
        return featureID in self.__getDataObj().index
...
class OldDatasetCollection:
    ...
    def __init__(self, dbManager, *args, **kwargs):
        """"""
        self.dbManager = dbManager  # DBFeatures
    ...
    def __contains__(self, dataID) -> bool:
        ...
        #
        # db_cur.execute(f"SELECT id FROM dataset WHERE label = '{dataID}';")
        # sql_result = db_cur.fetchone() is not None
        # db_cur.close()
        #
        # return sql_result  # dataID in self.collection # boolean
        return dataID in self.collection
    def __getitem__(self, dataID) -> OldDataset:
        """"""
        return self.collection.get(dataID)  # TODO: AUL not needed anymore, from database

    def __len__(self) -> int:
        """Returns the number of datasets"""
        return len(self.collection)  # TODO: AUL not needed anymore, from database

    def __str__(self) -> str:
        """"""
        return f"Total number of datasets : {self.getNumberOfDatasets()} - hash : ${self.__hash__()}"

    def items(self) -> List[Tuple[str, OldDataset]]:
        """Returns the items (dataID, Dataset)"""
        return self.collection.items()  # TODO: AUL not needed anymore, from database
    ...
    def addData(self,
        ...
        if newDataObj is not None:
            self.collection[dataID] = OldDataset(dataID, self.dbManager, newDataObj=newDataObj)
        else:
            # self.collection[dataID] = Dataset(dataID, data, params, self.dbManager)
            self.collection[dataID] = OldDataset(dataID, self.dbManager)  # TODO: AUL not needed anymore, from database

    def addDatasetFromPath(self,
                           dataID: str,
                           paramPath: str,
                           dataPath: str,
                           cmapDefaults: List[str],
                           overwriteCmapsByDefault: bool = True) -> None:
        ...
        #
        #     self.collection[dataID] = Dataset(dataID, data, params, self.dbManager)  # TODO: AUL not needed anymore, from database

    def getDataIDs(self) -> List[str]:
        """Returns the dataIDs"""
        return list(self.collection.keys())  # TODO: AUL not needed anymore, from database
    ...
    def values(self) -> List[OldDataset]:
        """"""
        return list(self.collection.values())  # TODO: AUL not needed anymore, from database
...
class OldData(object):
    ...
    """
    def __init__(self, pathToData, pathToAPIConfig, pathToArchive, dbManager: OldDBFeatures, *args, **kwargs):
        """"""
        # Todo: read from DB
        ...
        self.dataCollection = OldDatasetCollection(dbManager)  # DBFeatures
        self.__readConfig()
        self.__readData()
        self.__handleShortCutFilters()
        self.__readDataInfo()

    @deprecated(version="2.0", reason="Not required anymore after Data Update")
    def __readData(self) -> None:  # Todo: check if it is called twice as start? not necessary
    ...
    def __readDataInfo(self):
        """
        Creates an easy access summary dataframe to be shown as a summary (hence the name).
        Check for existence of datasets, if not creates an empy dataframe with required columns.
        """
    ...
    def __readConfig(self):
        ...
        # Todo: Outsource Configuration in own Singleton class
    ...
    def _getOneWayANOVASignificanceHits(self,
                                        anovaDetails: dict,
                                        propNames: list = ["anovaType", "pvalue", "grouping1"]):
        """"""
    ...
    def _performTwoWayANOVA(self,
                            ...
                            expColumns: List[str],
                            anovaDetails: dict,
                            groupings: dict, grouping1: str, grouping2: str,
    ...
    @deprecated(version="2.0", reason="Not required anymore after Data Update")
    def checkForMissingDataset(self) -> bool:
    ...
    def getDataSummary(self) -> pd.DataFrame:
        ...
        # TODO: Investigate TypeError: Object of type Timestamp is not JSON serializable
        # TODO: Rewrite method so that one has to give a list of ids or page number
        self.checkForMissingDataset()
    ...
    def getDataIDsInFolder(self) -> List[str]:
        ...
        # TODO: remove this method: getDataIDsInFolder
        ...
        if self.dataIDExists(dataID):
            ...
            if returnDataQuantiles:
                return (data.loc[idxIntersection, expColumns].median(axis=1),
                        np.nanquantile(data.values.flatten(), q=[0, 0.25, 0.5, 0.75, 1]))
            else:
                return data.loc[idxIntersection, expColumns].median(axis=1)
    ...
    def update(self) -> None:
        ...
        return
        # if not self.__checkData(): # TODO: Check if it should do something here!
        #     self.__readData()
        #     self.__readDataInfo()
    ...
    def getGroupingDetails(self, dataID: str, expColumns: list):
        ...
        if self.dataIDExists(dataID):
            ...
            groupingMapper = self.getGroupingMapper(dataID)  # ict [groupingName (Genotype)] -> {columnName (WT_01): groupName (WT)}
    ...
    def getMitoMapData(self, dataID, anovaDetails={"pvalue": 0.001}):
        ...
        if self.dataIDExists(dataID):
            ...
            mitoPaths = self.dbManager.getDBInfoForFeatureListByColumnName(X.index,
                                                                           mitoColumns[-1],
                                                                           checkShape=False).replace("0", np.nan).dropna()
            mitoGeneNames = self.dbManager.getDBInfoForFeatureListByColumnName(X.index,
                                                                               "Gene names (primary)",
                                                                               checkShape=False).dropna()
            expColumns = self.getExpressionColumns(dataID)
            ...
            if anovaType == "1-way ANOVA":
                ...
                boolIdx, _, _ = self._performTwoWayANOVA(X,
                                                         expColumns,
                                                         anovaDetails,
            ...
            for pathwayName, idxs in pathwayIDMatch.items():
                ...
                pathwayIDMatches[pathwayName] = sorted([{
                    ...
                    "idx": idx,
                    "sig": idx in boolIdx.values} for n, idx in enumerate(idxs)],
                    key=lambda d: d["sig"],
                    reverse=True)
    ...
    def _getBoxplotDataForMultipleFeatures(self, dataID: str, featureIDs: list) -> dict:
        ...
        boxplotData[quantileColumnName] = boxplotData[quantileColumnName].replace([0, 0.25, 0.5, 0.75, 1],
                                                                                  ["min", "q25", "m", "q75", "max"])

        v, legendTitle, legendData, tickLabel, legendItems = self._getDataForBoxplot(boxplotData, groupedData,
                                                                                     groupings, groupingNames,
                                                                                     groupingColorMapper,
                                                                                     quantileColumnName)
    ...
    def getHeatmapData(self, dataID: str, anovaDetails: dict = {}):
        ...
        if self.dataIDExists(dataID):
            ...
            if anovaType == "1-way ANOVA":
                ...
                boolIdx, selectionpvalues, pvalueNames = self._performTwoWayANOVA(X, expColumns, anovaDetails,
                                                                                  groupings, grouping1, grouping2,
                                                                                  anovaCutoff)

            if not np.any(boolIdx):
                return False, "Significance cutoff resulted in an empty data frame "\
                              "(e.g. no significantly different proteins)."
            ...
            rowLinkage = fastcluster.linkage(values,
                                             method="complete",
                                             metric="euclidean")
            maxD = 0.75*max(rowLinkage[:, 2])
            Z_row = sch.dendrogram(rowLinkage,
                                   orientation='left',
                                   color_threshold=maxD,
                                   leaf_rotation=90,
                                   ax=None,
                                   no_plot=True)
    ...
    def getVolcanoData(self, dataID: str, grouping: dict) -> Tuple[bool, dict]:
        ...
        if self.dataIDExists(dataID):
            ...
            expColumns = self.getExpressionColumns(dataID)
            paramNames = ["annotation-colum",
                          "filters-in-volcano-plots",
                          "highlight-in-volcano-plots",
            ...
            filterColumnDBInfo = [self.dbManager.getDBInfoForFeatureListByColumnName(data.index,
                                                                                     colName,
                                                                                     checkShape=False).fillna("-") for colName in filterColumns]

            annotationColumnInfo = [self.dbManager.getDBInfoForFeatureListByColumnName(data.index,
                                                                                       annotationColumn,
                                                                                       checkShape=False).fillna("-")]

            # attach DB info to data
            data = data.join(filterColumnDBInfo + annotationColumnInfo)

            # if missing features in DB that are present in data, NaN are created, replace them.
            ...
            if len(highlightColumns) > 0:
                ...
                for highlightColumn in highlightColumns:
                    ...
                    vs = dict([(cat if highlightColumn not in highlightColumnSepForMenu else cat.split(">")[-1],
                                data.index[data[highlightColumn].str.contains(buildRegex([cat], splitString=";")).fillna(False)].values.tolist()) for cat in flatSplitDataList if cat not in ["-", "0"]])
                    if 'nan' in vs:
                        del vs['nan']
        ...
        if len(groupingNames) == 1:
            ...
            for groupName1 in groupings[groupingNames[0]].keys():
                ...
                for groupName2 in groupings[groupingNames[1]].keys():
                    try:  # Todo: Fix Bug group name combination not in groupedBoxData, how to exclude? what to do with this?
                        groupData = groupedBoxData.get_group((groupName1, groupName2))
                    except:
                        print(" <BUG> group1 group2 combination that does not exist in dataset")
                        continue
    ...
    def getDataForCard(self, dataID: str, featureID: list, filterName: str) -> Dict[str, Any]:
        ...
        quantileColumnName = "level_{}".format(len(groupingNames))  # get name for quantile
        quantileData[quantileColumnName] = quantileData[quantileColumnName].replace([0, 0.25, 0.5, 0.75, 1],
                                                                                    ["min", "q25", "m", "q75", "max"])
        try:
