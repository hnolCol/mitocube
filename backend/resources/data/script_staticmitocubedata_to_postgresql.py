...
SQL_TEMPLATE_OI_ALL = "SELECT id, label FROM oi;"
SQL_TEMPLATE_OI = "INSERT INTO oi(label, type) VALUES ('%s', 'Protein');"
...
for item in list_dataset_ids:
    ...
    try:
        # list and insert of missing OIs using SQL_TEMPLATE_OI_ALL #####################################################
        db_cur.execute(SQL_TEMPLATE_OI_ALL)
        db_ois = db_cur.fetchall()
        db_ois = pd.DataFrame(db_ois, columns=["id", "label"])
        db_ois["label"] = db_ois["label"].apply(lambda x: x.rstrip())  # Todo: fix white space in DB

        db_ois_missing = [oi not in db_ois["label"].tolist() for oi in in_data.index]
        # print("\n%d\n" % sum(db_ois_missing))
        to_execute_oiInsert = "\n".join([SQL_TEMPLATE_OI % x for x in in_data.index[np.where(db_ois_missing)].tolist()])
        # db_cur.execute(to_execute_oiInsert) # ToDo: Activate me, check access rights for sequences in DB
        ...
        print("")
        # print(in_meta)
        # print(in_meta["Experimental Info"])
        # print([item["title"] for item in in_meta["Experimental Info"]])
        print(np.where([title == "Research Aim" for title in [item["title"] for item in in_meta["Experimental Info"]]])[0][0])
        print(np.where([title == "Additional Information" for title in [item["title"] for item in in_meta["Experimental Info"]]])[0][0])
        print(np.where([title == "Sample Preparation" for title in [item["title"] for item in in_meta["Experimental Info"]]])[0][0])
        # print(in_meta["Type"]["details"])
        # print(in_meta["Data Analysis"]["details"])
        print(SQL_TEMPLATE_DS % ("NULL",  # project_id
                                 in_meta["dataID"],  # dataset id / label, unique!
                                 1,  # instrument id # ToDo: change instrument ID
                                 in_meta["Creation Date"],  # created_on # ToDo: check format, datetime.strptime(datetime_str, '%Y%m%d %H:%M:%S').strftime("%Y-%m-%d %H:%M:%S")
                                 datetime.now().strftime("%Y-%m-%d %H:%M:%S"),  # uploaded on # ToDo: check format
                                 "submitted",  # DATASET_STATE: submitted hidden
                                 in_meta["Experimentator"],  # experimentator
                                 "ignotus.homo@eremus.ob",  # contact_email  # ToDo: get specific email from experimentator
                                 "AG Ignotus",  # name_group
                                 in_meta["Title"],  # title
                                 "To define is to limit.",  # research_question # ToDo: Outsource
                                 "ignotus",  # protein_of_interest # ToDo: Outsource
                                 in_meta["Organism"],  # organism  # ToDo: Translate in_meta["Organism"] to ORGANISM
                                 in_meta["Type"],  # type
                                 in_meta["Material"],  # material
                                 "I am not young enough to know everything.",  # research_aim # ToDo: Outsource
                                 "I have nothing to declare except my genius.",  # research_sample_preparation # ToDo: Outsource
                                 "Experience is merely the name men gave to their mistakes."  # research_information # ToDo: Outsource
                                 ))
        print("")
        raise Exception("Not so fast Mister!")

        (project_id, label, instument_id, created_on, uploaded_on, state,
        experimentator, contact_email, name_group, title, research_question,
        protein_of_interest, organism, type, material, research_aim,
        research_sample_preparation, research_information)
