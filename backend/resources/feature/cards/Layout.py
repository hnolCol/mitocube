    ...
    def post(self):
        ...
        if request.data != b'': 
            ...
            if all(x in data for x in ['token','featureIDs','columnNumber']):
                ...
                dataIDsByFeature = self.featureFinder.getDatasets(featureIDs ,dataIDFilter)
                ...
                for featureID in featureIDs:
                    ...
                    for bp, columnNumber in columnNumberBreakpoints.items():
                        ...
                        #     "i":k["id"],
                        #     "x":n,
                        #     "y": int((n+2)/columnNumber),
                        #     "w":2,
                        #     "h":2,
                        #     "static":False} for n,k in enumerate(cs)]
                ...
                return {"layout":layouts,"cards":cards,"filter":shortCutFiltersByFeatures,"filterColors":shortCutFilterColors,"activeFilter":[]}
            
        return {"error":"Error not sufficient data provided. JSON data must contain 'token','featureIDs' and 'columnNumber'"}