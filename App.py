...
from backend.helper.PTM import PTMManger
...
# ToDo: pathToAPIConfig should point to figure, and check for existence!
...
staticDatasets, staticUniprot = os.path.join(pathToStaticData, "datasets"), \
                                             os.path.join(pathToStaticData, "dbs", "uniprot")
pathToPerformance = os.path.join(pathToDynamicData, "performance")
pathToSubmission = os.path.join(pathToDynamicData, "submissions")


# creating folders if they do not exist, should actually only happen on first start. ###################################
...
dataManger = OldData(pathToData, pathToAPIConfig, pathToArchive, dbManager)
...
for k, v in emailSettings.items():
    if v in [0, 1]:
        app.config[k] = v == 1  # 0 and 1 are transferred to True/False, might wort also with 0/1?
    else:
        app.config[k] = v
