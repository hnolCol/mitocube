                ...
                return(
                    <div key={`clust${clusterIndex}`} >
                        <svg width={widthForGroupings+10} height={110} 
                            ...
                            <rect x={0} y={0} width={widthForGroupings} height={110} fill="#e0e0e0" />
...
function MCMinimalClusterView(props) {
    ...
    const defaultStrokeWidth = selected?0.75:0.25
    ...
    return (
        <g>
            ...
            <Text
                x={margin.left + 21}
                y={3} fill={selected ? textColor : "black"}
                fontWeight={selected ? 500 : 350}
                textAnchor={"middle"}
            ...
            <line x1 = {margin.left} x2={width-margin.right} y1={yscale(0)} y2={yscale(0)} stroke="black" strokeWidth={defaultStrokeWidth}/>
            <line x1 = {margin.left} x2={margin.left} y1={minYValue} y2={yscale(maxValue)} stroke="black" strokeWidth={defaultStrokeWidth}/>
            ...
            {vs.map((p, i) => {
                ...
                return(
                    ...
                    {mouseOver || selected ? groupingNames.map((groupingName, ii) => {
                        ...
                        let maxValueScaled = yscale(-maxValue)
                            return(
                                <g key = {`${ii}${groupingName} - legen bars label`}>
                                    <rect
                                        x={barX}
                                        y={maxValueScaled + 5 + heightGroupingRect * ii}
                                        width={barWidth}
                                ...
                                {mouseOverBarIndex!==undefined&&mouseOverBarIndex===i?
                                    <Text 
                                                x = {xtext} 
                                                y = {maxValueScaled + 5 + heightGroupingRect * ii + heightGroupingRect/2} 
